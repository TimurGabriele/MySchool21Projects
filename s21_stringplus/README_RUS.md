...
В данном проекте разработана собственная реализация библиотеки string.h на языке программирования С с некоторыми дополнениями (с собственной реализацией функций sprintf и sscanf). 
...
- Стандартная библиотека string.h и другие библиотеки по обработке строк используются только в unit-тестах;
- Не используются системные списки ошибок, включая списки, непрописанные в стандартах POSIX (sys_nerr, sys_errlist). Вместо них реализованы свои платформозависимые списки ошибок, как это было упомянуто в описании функции *strerror*;
...
### Дополнительно. Реализация некоторых модификаторов формата функции sprintf:
